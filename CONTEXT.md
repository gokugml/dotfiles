...
共享配置、个人配置和本机私有参数按固定顺序加载后的运行时结果。
...
Stage 1 通过独立 Skill 把已确认的 `zsh-repair-plan.md` 应用到用户显式提供的 `zshrc`/`.zshrc`、`zprofile`/`.zprofile` 目标；没有显式目标时先让用户选择无前置点或有前置点仓库命名，再更新对应文件和可选 shared 增量。它尽可能保留现有 Oh My Zsh 模板配置，不建立 symlink 或安装软件。
...
Zsh 加载顺序固定为 `shared → personal → local`，因此覆盖优先级为 `shared < personal < local`。
