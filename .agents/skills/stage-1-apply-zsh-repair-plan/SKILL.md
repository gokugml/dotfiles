...
`~/.config/dotfiles/local/integrations.zsh` 和 `parameters.zsh` 是本 Skill 仅有的固定 HOME 私有数据例外，不属于仓库 Zsh 入口目标。只有修复计划要求迁移功能块或私有赋值、执行前计划已逐项展示且最新 diff 已确认时才允许更新；不得借此修改其他 HOME 文件。
