...
1. 对比已确认源 Zsh 功能块/PATH 与 Stage 1 候选，只处理本次变更明确移除、替换或改变语义的 runtime initializer、global home 和 PATH 目录。
2. 覆盖 npm、pnpm、Bun，以及这些被移除目录中的直接可执行文件；不要扫描无关 PATH、整个 HOME、`/usr/local` 或项目目录。
...
1. 在展示 Zsh 完整 diff 前完成只读发现，并在执行前计划中列出将检查的精确旧 owner/目录。
