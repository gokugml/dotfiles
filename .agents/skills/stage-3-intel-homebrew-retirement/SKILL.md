...
先只读检查仓库、工作树、Stage 2 完成证据、personal/company 配置、目标架构和安装器命令面；不得运行 `retire --apply`、卸载命令或删除任何内容。随后向用户展示计划：将运行的只读 verify/retire 检查、可能进入候选删除集的类别、明确保留类别、真实软件与路径可能受到的影响、不可自动恢复风险、后续删除确认和验证方式，以及明确不会安装替代、处理服务数据、删除未知 `/usr/local` 内容、commit 或 push。展示后停止并等待用户明确确认，再进入只读预检和预览。
...
1. 定位当前公开仓库根目录，确认 `install.sh` 存在且 Stage 2 使用的 personal/company 配置仍能唯一确定；歧义会改变退役集合时停止并询问。
