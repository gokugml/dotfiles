...
> 项目关系：本文是 [四阶段共用契约](../../stage-common-contract.md) 的按需诊断参考；阶段职责以 [四阶段流程图](../../steps.excalidraw) 和对应阶段 Skill 或文档为准。

## 目录

- [文档背景与目的](#文档背景与目的)
- [1. 为什么要诊断 `.zshrc`](#1-为什么要诊断-zshrc)
- [2. 修改建议的分类](#2-修改建议的分类)
- [3. Zsh 启动文件的职责边界](#3-zsh-启动文件的职责边界)
- [4. Apple Silicon 与 Intel 迁移诊断](#4-apple-silicon-与-intel-迁移诊断)
- [5. `PATH`：顺序、唯一性与幂等性](#5-path顺序唯一性与幂等性)
- [6. 功能正确性与防御式加载](#6-功能正确性与防御式加载)
- [7. 补全、插件与初始化顺序](#7-补全插件与初始化顺序)
- [8. 启动性能](#8-启动性能)
- [9. 变量作用域](#9-变量作用域普通变量export-与命令级注入)
- [10. 密钥与敏感信息](#10-密钥与敏感信息)
- [11. 工具职责与版本管理](#11-工具职责与版本管理)
- [12. 公开仓库、个人配置与私有参数](#12-公开仓库个人配置与私有参数)
- [13. 安装脚本与本地副本](#13-安装脚本与本地副本)
- [14. 推荐的只读诊断流程](#14-推荐的只读诊断流程)
- [15. 诊断报告的推荐格式](#15-诊断报告的推荐格式)
- [16. 最终验收清单](#16-最终验收清单)
- [17. 参考资料](#17-参考资料)
...
本文源于一次从 Intel Mac 迁移到 Apple Silicon 后的 dotfiles 整改需求。旧 `.zshrc` 长期叠加了安装器生成片段、版本管理器、补全、插件、工具路径和私有环境变量。机器迁移后，这些配置可能继续加载 Intel Homebrew、让多个管理器争夺同一运行时，或把本机路径和密钥与可公开配置混在一起。

这类问题往往不会引发明显的 Zsh 语法错误，而是以命令来源错乱、架构混用、启动变慢、补全重复初始化、换机后失效或敏感信息暴露的形式出现。因此，在实施迁移和退役 Intel Homebrew 之前，需要先建立一套只读、有证据、可重复的诊断基线。
...
- Zsh 证据采集脚本和 AI 不采集其内容；`install.sh` 不打印、复制或持久化内容，只允许语法检查和正常 shell 加载；
...
报告应从“先看懂全貌”再进入细节，固定使用以下顺序：

1. `结论`：3–6 条，包含已确认正常状态、最重要的 P0/P1 和关键证据缺口；
2. `问题一览`：一行一个根因，按优先级和证据等级排序；
3. `详细发现`：展开证据、影响、建议、风险和验收；
4. `已确认正常或无实际影响`：记录语法通过、注释项不生效、缺失文件无须创建等保留项；
5. `证据缺口` 与 `自检结果`。
...
|---|---|---|---|---|---|
| P0/P1/P2/P3/保留 | 已确认/结构信号/证据缺口 | `.zshrc:line N` 等脱敏逻辑位置 | 一句话 | personal/local/manual 等 | 一句话 |

证据等级的含义：

- `已确认`：只读运行时事实，或足以直接支持结论的确定性结构事实；
- `结构信号`：发现了非注释行、顺序、计数或类别，但没有观察分支执行与运行结果；
- `证据缺口`：当前安全采集没有覆盖，必须写明需要什么命令或人工核验才能升级结论。

一览表必须覆盖所有详细发现，但不要把同一根因拆成多行。非行动项使用 `保留`，不伪装成低优先级问题。public 报告的位置使用逻辑文件名和行号，不链接真实 HOME 绝对路径。

每个详细发现都应包含证据、影响、建议和验收方式，而不只是给出“最佳实践”结论。
