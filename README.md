...
- Apple Silicon 上如果仍有 Intel 软件或路径，`verify` 在 `${XDG_STATE_HOME:-$HOME/.local/state}/dotfiles/intel_to_be_retired.tsv` 写入 `0600` 的确定性交接清单；它不进入 Git，也不是删除授权。
...
- 旧 Intel 路径可以保留，但不得成为任一受管命令或 symlink 的最终目标；每个残留项的精确路径和保留理由进入 `intel_to_be_retired.tsv`，由 Stage 3 重新核验。
